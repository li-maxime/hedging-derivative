...
class Archive(object):
    ...
    def __init__(self, archiveFile: str):
        ...
        if not filePath.is_file():
            print("\tFile {} is not a recognized archive".format(archiveFile))
            raise ValueError
    ...
    def check(self) -> bool:
        ...
        print("\tToplevel directory: {}".format(self.toplevelDir))
        ...
        for f in self.filePaths:
            if f.is_absolute() or f.parts[0] == '..':
                print("\t{} refers to a file outside the toplevel directory".format(f.as_posix()))
                return False
    ...
    def extract(self):
        ...
        if destdir.exists() and not destdir.is_dir():
            print("\t{} already exists but is not a directory".format(destdir))
            raise ValueError
...
def build_project(project) -> bool:
    ...
    try:
        ...
        for e in EXECS:
            ...
            print("\tChecking if {} is present...".format(e), "OK" if exec_exists else "FAIL")
        return True
...
def copy_execs(project):
    ...
    if structuredir.exists() and not structuredir.is_dir():
        print("\t{} already exists but is not a directory".format(structuredir))
        raise ValueError
    ...
    for e in EXECS:
        ...
        if exec_path.exists():
            ...
            print("\tCopying {} to {}".format(exec_path.as_posix(), dest_exec.as_posix()))
...
if __name__ == "__main__":
    ...
    for arg in args_list:
        ...
        if check:
            ...
            print("\tChecking archive format... {}".format("OK" if archive_isvalid else "FAIL"))
        if extract:
            print("\tExtracting {} to {}".format(arg, destdir))
            archive.extract()
